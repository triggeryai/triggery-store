...
const FaqDetails: React.FC = () => {
  return (
    <section className="py-10">
      ...
      <ul className="max-w-2xl mx-auto divide-y shadow-lg rounded-xl">
        {faqData.map((faq, index) => (
          <li key={index}>
            <details className="group p-5">
              <summary className="flex items-center justify-between cursor-pointer font-medium text-lg transition-all duration-300 group-hover:text-blue-500">
                <span>{faq.question}</span>
                <FiChevronDown className="w-6 h-6 transition-transform duration-300 group-open:rotate-180" />
              </summary>
